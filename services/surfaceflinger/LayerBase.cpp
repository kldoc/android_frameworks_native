...
void LayerBase::drawWithOpenGL(const sp<const DisplayDevice>& hw, const Region& clip) const
{
    ...
    if(hw->setDispProp(DISPLAY_CMD_GETDISPLAYMODE,0,0,0) == DISPLAY_MODE_SINGLE_VAR_GPU)
    {
        ...
        int valid_width = hw->setDispProp(DISPLAY_CMD_GETDISPPARA,0,DISPLAY_VALID_WIDTH,0);
        int valid_height = hw->setDispProp(DISPLAY_CMD_GETDISPPARA,0,DISPLAY_VALID_HEIGHT,0);
        int vp_w = hw->getWidth() * valid_width / app_width;
        int vp_h = hw->getHeight() * valid_height / app_height;
        int scissor_w = hw->getWidth();
        int scissor_h = hw->getHeight();

        if(valid_width > app_width)
        {
            scissor_w = hw->getWidth()*valid_width/app_width;
        }
        if(valid_height > app_height)
        {
            scissor_h = hw->getHeight()*valid_height/app_height;
        }
        ...
        else if(hw->getOrientation() == 1)
        {
            if(hw->setDispProp(DISPLAY_CMD_GETDISPPARA,0,DISPLAY_OUTPUT_TYPE,0) != DISPLAY_DEVICE_LCD)
            {
                ...
                vp_w = hw->getWidth() * valid_width / app_width;
                vp_h = hw->getHeight() * valid_height / app_height;
            }
